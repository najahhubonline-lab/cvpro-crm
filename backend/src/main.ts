...
async function bootstrap() {
  ...
  app.use(helmet({
    contentSecurityPolicy: false, // Disabled to allow React frontend to load inline scripts/styles if needed
  }));
  app.use(compression());
  ...
  if (process.env.WORKER_MODE === 'true') {
    await app.init();
    logger.log('Worker mode started. HTTP server disabled.');
    return;
  }
